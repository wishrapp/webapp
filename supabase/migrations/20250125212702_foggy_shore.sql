/*
  # Fix Occasions Table Migration
  
  1. Changes
    - Drop existing triggers and functions
    - Create occasions table with proper case sensitivity
    - Add RLS policies with unique names
    - Add default occasions function and trigger
...
CREATE POLICY "occasions_select_own_20250125"
ON "occasions" FOR SELECT
USING (auth.uid() = user_id);

CREATE POLICY "occasions_select_shared_20250125"
ON "occasions" FOR SELECT
USING (
  EXISTS (
...
CREATE POLICY "occasions_insert_20250125"
ON "occasions" FOR INSERT
WITH CHECK (auth.uid() = user_id);

CREATE POLICY "occasions_update_20250125"
ON "occasions" FOR UPDATE
USING (auth.uid() = user_id);

CREATE POLICY "occasions_delete_20250125"
ON "occasions" FOR DELETE
USING (auth.uid() = user_id AND NOT is_default);

CREATE POLICY "occasions_admin_20250125"
ON "occasions"
USING (is_admin());
