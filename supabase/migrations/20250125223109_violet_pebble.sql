...
DROP POLICY IF EXISTS "occasions_policy_select_own" ON "occasions";
DROP POLICY IF EXISTS "occasions_policy_select_shared" ON "occasions";
DROP POLICY IF EXISTS "occasions_policy_insert" ON "occasions";
DROP POLICY IF EXISTS "occasions_policy_update" ON "occasions";
DROP POLICY IF EXISTS "occasions_policy_delete" ON "occasions";
DROP POLICY IF EXISTS "occasions_policy_admin" ON "occasions";
DROP POLICY IF EXISTS "occasions_select_own_20250125" ON "occasions";
DROP POLICY IF EXISTS "occasions_select_shared_20250125" ON "occasions";
DROP POLICY IF EXISTS "occasions_insert_20250125" ON "occasions";
DROP POLICY IF EXISTS "occasions_update_20250125" ON "occasions";
DROP POLICY IF EXISTS "occasions_delete_20250125" ON "occasions";
DROP POLICY IF EXISTS "occasions_admin_20250125" ON "occasions";
...
CREATE POLICY "occasions_select_own_223035"
ON "occasions" FOR SELECT
USING (auth.uid() = user_id);

CREATE POLICY "occasions_select_shared_223035"
ON "occasions" FOR SELECT
USING (
  EXISTS (
...
CREATE POLICY "occasions_insert_223035"
ON "occasions" FOR INSERT
WITH CHECK (auth.uid() = user_id);

CREATE POLICY "occasions_update_223035"
ON "occasions" FOR UPDATE
USING (auth.uid() = user_id);

CREATE POLICY "occasions_delete_223035"
ON "occasions" FOR DELETE
USING (auth.uid() = user_id AND NOT is_default);

CREATE POLICY "occasions_admin_223035"
ON "occasions"
USING (is_admin());

-- Create function to add default occasions with null date handling
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    -- Insert "Birthday" only if date_of_birth is not null
...
-- Add default occasions for existing users with null check
DO $$
DECLARE
  user_record RECORD;
BEGIN
