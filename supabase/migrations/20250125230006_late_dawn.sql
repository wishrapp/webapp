...
CREATE POLICY "occasions_select_230012"
ON occasions FOR SELECT
USING (auth.uid() = user_id);

CREATE POLICY "occasions_select_shared_230012"
ON occasions FOR SELECT
USING (
  EXISTS (
...
CREATE POLICY "occasions_insert_230012"
ON occasions FOR INSERT
WITH CHECK (auth.uid() = user_id);

CREATE POLICY "occasions_update_230012"
ON occasions FOR UPDATE
USING (auth.uid() = user_id);

CREATE POLICY "occasions_delete_230012"
ON occasions FOR DELETE
USING (auth.uid() = user_id AND NOT is_default);

CREATE POLICY "occasions_admin_230012"
ON occasions
USING (is_admin());