...
interface EditItemFormProps {
  item: Item;
  occasions: Occasion[];
  onSubmit: (itemId: string, formData: any) => Promise<boolean>;
  onClose: () => void;
}

export default function EditItemForm({ item, occasions, onSubmit, onClose }: EditItemFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const success = await onSubmit(item.id, formState);
      if (success) {
        onClose();
      }
    } catch (error) {
